...
class MumbleIceApp(Ice.Application):
    ...
    def record_channel_activity(self, channel_id):
        ...
        channel-activity callback on connect / channel-hop, and by the
    ...
    def attach_server_callback(self, server):
        """Register ChannelActivityCallback so the cleanup job sees activity.
        ...
        cb = ChannelActivityCallback(self.app, self.logger, server)
        ...
        try:
            ...
            self.logger.info(f"Channel activity callback attached to server {server_id}")
    ...
    def sync_channels_from_groups(self, server):
        ...
        Channel name == group name so the cleanup job can preserve channels
        backed by a group.  Skips __ANON__.  Never deletes channels —
        too risky if users are mid-conversation; logs instead.
        """
        try:
    ...
    def _ensure_temp_channel_acls(self, server, managed_names):
        ...
        Temp channels themselves rely entirely on inherited ACLs from their
        parent — no per-temp setACL hook is needed.
        """
        try:
...
class ChannelActivityCallback(Murmur.ServerCallback):
    """Records channel activity so the cleanup job can identify idle channels.

    Channel access and speak rights are controlled entirely by Murmur ACLs
    (server group @<groupname> grants Enter+Speak to the channel).  This
    callback does not inspect or enforce OTS group direction — IN and OUT
    members both speak freely once their server-group membership lets them
    enter the channel.
    """
