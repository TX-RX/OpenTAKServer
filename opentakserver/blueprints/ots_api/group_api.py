...
def _request_mumble_channel_sync():
    ...
    Why: each OTS group should have a matching Mumble channel so members
    can use voice chat for that group. Silent no-op if the Ice daemon
    isn't running.
    """
    try:
    ...
    if app.config.get("OTS_ENABLE_LDAP"):
        ...
        for group in groups:
            if group["cn"].lower().startswith(
                app.config.get("OTS_LDAP_GROUP_PREFIX").lower()
            ) and not (
