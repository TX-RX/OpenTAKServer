...
# GUID-routed alias for delete_invitation. CloudTAK/node-tak only hits the GUID
# variant; without this the revoke-invite button 404s.
@mission_marti_api.route(
    "/Marti/api/missions/guid/<mission_guid>/invite/<invitation_type>/<invitee>", methods=["DELETE"]
)
def delete_invitation_by_guid(mission_guid: str, invitation_type: str, invitee: str):
    mission = db.session.execute(db.session.query(Mission).filter_by(guid=mission_guid)).first()
    if not mission:
        return (
            jsonify({"success": False, "error": gettext("Mission not found: %(g)s", g=mission_guid)}),
            404,
        )
    return delete_invitation(mission[0].name, invitation_type, invitee)
...
def get_mission_cots(mission_name: str = None, mission_guid: str = None):
    ...
    <events> tag. KML/KMZ files attached to the mission are parsed and emitted as CoT routes/markers so
    they render on TAK clients' maps.
    """
    ...
    # Augment with CoT events derived from KML attachments. KML/KMZ files
    # uploaded as mission contents do not get into the CoT table by default;
    # parse them here so their routes/points/polygons render on the map.
    if mission_name:
        for kml_event in _kml_events_for_mission(mission_name):
            events.append(kml_event)
...
def _kml_events_for_mission(mission_name: str):
    """Yield CoT events generated from every KML/KMZ attachment on this mission."""
    from opentakserver.kml_to_cot import kml_attachment_to_cot_events
    ...
    missions_dir = os.path.join(app.config.get("OTS_DATA_FOLDER"), "missions")
    for row in attached:
        content = row[0]
        if not _is_kml_mime(content.mime_type, content.filename):
            continue
        path = os.path.join(missions_dir, content.filename)
        try:
            with open(path, "rb") as fh:
                data = fh.read()
        except (FileNotFoundError, OSError) as exc:
            logger.warning("KML attachment unreadable for mission %s: %s -> %s", mission_name, path, exc)
            continue
        try:
            for ev in kml_attachment_to_cot_events(data, content.hash):
                yield ev
        except Exception:
            logger.exception("Failed to convert KML attachment to CoT: %s", content.filename)


def _is_kml_mime(mime_type: str, filename: str) -> bool:
    if mime_type and "kml" in mime_type.lower():
        return True
    if filename:
...
# node-tak (used by CloudTAK) calls GET /Marti/api/missions/<name>/layers and
# expects a TAKList-shaped JSON body. We return one MissionLayer per attached
# KML/KMZ — each layer groups all the CoT events generated by KML→CoT in /cot.
# Without this grouping, CloudTAK's Mission tree shows every route point + POI
# as a separate orphan entry.
@mission_marti_api.route("/Marti/api/missions/<mission_name>/layers", methods=["GET"])
def get_mission_layers(mission_name: str):
    data = list(_kml_layers_for_mission(mission_name))
    return jsonify({"version": "3", "type": "MissionLayer", "data": data, "nodeId": ""})


def _kml_layers_for_mission(mission_name: str):
    """Yield one MissionLayer dict per KML/KMZ attachment on this mission."""
    from opentakserver.kml_to_cot import kml_attachment_to_layer
    ...
    for row in attached:
        ...
        try:
            ...
            continue
        try:
            layer = kml_attachment_to_layer(
                data, content.hash, content.filename,
                creator_uid=content.creator_uid or "kml-attachment",
            )
            if layer:
                yield layer
        except Exception:
