...
ATAK device  ──TLS──>  Murmur (mumble-server)
                       ...
                       └── ChannelActivity       (records activity for cleanup job)
```
...
2. Registers a per-server `ChannelActivityCallback` that records channel
   activity timestamps for the cleanup job (see "Channel cleanup" below).
   The callback does **not** inspect OTS group direction — IN and OUT
   members both speak freely once their server-group membership lets
   them enter the channel.
...
Activity is recorded in-memory by `ChannelActivityCallback` on every
...
| Channel activity tracking (feeds the cleanup job) | `opentakserver/mumble/mumble_ice_app.py::ChannelActivityCallback` |
